...
// Set a higher body size limit for video uploads (100MB)
export const config = {
  api: {
    bodyParser: {
      sizeLimit: "100mb",
...
export async function POST(request: Request) {
  try {
    ...
    if (file.size > 100 * 1024 * 1024) {
      return NextResponse.json({ success: false, error: "File too large. Maximum size is 100MB." }, { status: 400 })
    }
    ...
    if (bucketsError) {
      ...
      return NextResponse.json(
        { success: false, error: "Failed to check storage buckets", details: bucketsError.message },
        { status: 500 },
      )
    }
    ...
    if (!videosBucketExists) {
      console.log("Videos bucket does not exist, creating it...")
      ...
      if (createBucketError) {
        ...
        return NextResponse.json(
          { success: false, error: "Failed to create videos bucket", details: createBucketError.message },
          { status: 500 },
        )
      }
